...
def launch_spector():
    ...
    if command:
        ...
        try:
            build_dir = os.path.expanduser('~/Testbed/OAI-5G-nr.attack.v2.1.0/cmaketargets/ranbuild/build')
...
def launch_replay():
    ...
    if pcap_file:
        try:
            replay_dir = os.path.expanduser('~/Testbed/5greplay')
...
def manual_modify():
    try:
        modify_dir = os.path.expanduser('~/Testbed/pcap_modifier')
