...
def save_log(container_name):
    ...
    filename = f"~/Testbed/logs/{timestamp}_{container_name}.log"
...
def save_pcap(container_name):
    timestamp = datetime.now().strftime("%Y%m%d_%H%M%S")
    filename = f"~/Testbed/pcaps/{timestamp}_{container_name}.pcap"
...
def capture_core():
    ...
    pcap_file = f"~/Testbed/pcaps/{date_string}_core.pcap"
    command = f"sudo tshark -i demo-oai -f 'not host 192.168.70.154 and not host 192.168.70.155' -w {pcap_file}"
    #command = f"sudo tshark -i demo-oai -f '!(host 192.168.70.154 or host 192.168.70.155)' -w {pcap_file}"
